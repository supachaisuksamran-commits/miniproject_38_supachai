...
$s = "SELECT * FROM model WHERE model_id = '$id' ";
$result = mysqli_query($con, $s);
$o = mysqli_fetch_assoc($result);

?>

    <form action="action/update_model.php" method="post">

    <label for="">ชื่องาน</label>
<input type="text" name="job_name" value="<?= $o["job_name"] ?>" > <br>

    <label for="">ขนาด</label>
<input type="text" name="size" value="<?= $o["size"] ?>" > <br>

    <label for="">เพศ</label>
<input type="text" name="geender" value="<?= $o["geender"] ?>" > <br>
...
<label for="">chose anime</label>
<select name="anime_id" id="">
<?php
  foreach($result as $a){
    ?>
    <option value="<?= $a["anime_id"] ?>">
        <?= $a["title"] ?>
    </option>
    <?php
  }
...
<input type="hidden" name="model_id" value="<?= $o['model_id'] ?>">
<br>    
